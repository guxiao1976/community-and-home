...
校验阶段 1 产物齐备，残缺输入会带偏设计：

1. `.harness/changes/<name>/proposal.md` 存在且非空
2. `specs/*/spec.md` 至少 1 个且非空
3. 无 `[NEEDS CLARIFICATION]` / `TBD` / `TODO` 占位符残留（有则 RETURN_TO_OWNER，不硬设计）

不合格 → 输出《输入不完整说明》提交 Owner，不进入设计。

### Step 1: 加载上下文

按顺序（与 `.harness/agents/subagents/architecture-designer.md` 加载清单对齐，单点维护）：

1. 阶段 1 产出 — `.harness/changes/<name>/request.md` + `proposal.md` + 所有 `specs/*/spec.md`
2. 受影响服务的 `docs/design.md` — 现有数据模型和接口
3. 受影响服务的 `docs/graph-context.md` — 技术清单（API 路由/gRPC 接口/数据表/服务依赖，Neo4j 自动生成）——架构设计必读，避免重复造轮子/与现状冲突
4. 根 `CLAUDE.md` + `.harness/rules/工程结构.md` — 全局架构约束
5. `api-proto/api/` — 现有 Proto 定义，避免重复或冲突
6. `.harness/knowledge/memory/MEMORY.md` — 精读相关架构决策记忆
...
2.5 **slug 自验** — 每个待注入的 `[[memory-slug]]` 注入前 Read/Grep 确认文件存在；引用不存在的 slug 会在 Review M2 报 CRITICAL，与其被动等 Review 发现不如注入时自验
...
5. **输出注入报告**：
   ...
   > 不适用 K 个 + 排除理由也写进 design.md「记忆引用」章节，便于 reviewer 快速确认是主动排除而非遗漏。
...
**归属存疑处理**：归属灰色地带（放 A 还是 B 有争议）不强行拍板——在归属表标注「待定 + 候选方案 + 推荐」，供设计评审裁决，而非留到编码阶段返工。
...
| 需求 ID（spec 条目） | 需求内容摘要 | 对应设计章节 | 覆盖状态 |
|---------------------|-------------|-------------|:---:|
| REQ-<capability>-1  | <摘要>      | §数据模型    | ✅ |

> 所有 spec 正式需求点 100% 覆盖；无对应 spec 的设计内容 = 设计蔓延，标注并说明理由。
...
CREATE TABLE <table_name> (
    ...
    deleted_at DATETIME NULL COMMENT '软删除（遵循编码规范 §5.1 时间字段三件套）'
);
```

**字段约束补充**（DDL 之外的必要信息）：
- 每个字段的业务含义、数据类型精度、非空/唯一约束、枚举值范围
- 表引擎/字符集（InnoDB/utf8mb4）、软删除策略（是否 deleted_at 逻辑删除）
- 数据增长预估 + 归档/清理策略（如日志表保留周期）
- 表关系：与现有表的关联、外键在业务层还是数据库层
...
- **输入**：<字段列表>（含必填/边界/格式校验规则，映射自 spec 场景）
- **输出**：<字段列表>
- **鉴权**：<所需权限 / 是否用户态 / 是否内部免鉴权>
- **幂等**：<是否需要幂等 + 幂等键定义>
- **性能约束**：<超时时间 / 限流阈值 / 预期耗时>
- **错误码**：<5位错误码>（每个错误码对应的业务场景 + 触发条件）
...
**跨服务一致性**：<涉及多服务数据时的一致性方案：最终一致性/Saga/事务消息 + 失败补偿>
...
> 破坏性变更须补「影响评估」：影响哪些调用方 + 兼容过渡期/灰度方案/回滚策略。
...
- [ ] 可靠性：接口幂等 / 异常重试 / 降级熔断 / 数据回滚
- [ ] 性能：分页 / 缓存策略（key+过期+一致性）/ 限流 / 大查询优化 / 索引评估
- [ ] 可观测性：核心链路日志埋点 / 关键指标（QPS/耗时/错误率）/ 告警阈值
- [ ] 无显式要求则标注「无」
...
```
### Step 3.5: Design Self-Review（产出 design.md 后自检，先于拆任务）

在拆 tasks 之前校验 design.md 本身质量（设计错误会传导到任务，返工成本高）：

1. **追溯矩阵**：所有 spec 需求点是否 100% 覆盖 design？有无无 spec 依据的设计蔓延？
2. **服务归属**：符合数据所有权原则？存疑项是否标注「待定+候选+推荐」？
3. **数据模型**：Snowflake ID / created_at+updated_at+deleted_at / 软删除 是否符合编码规范？
4. **非功能**：可靠性/性能/可观测性是否完整或显式标注「无」？
5. **破坏性变更**：Proto/DB 破坏性变更是否标注 + 评估影响？
6. **记忆引用**：已注入 + slug 自验存在？不适用项已记录排除理由？

发现问题 → 先修正 design.md，再拆任务。
...
**触发条件**：仅当 Owner 明确要求走 Ralph 模式（单服务执行 Agent 读取）时才生成；默认 L 级 spec-pipeline 流程不生成，主流程以 tasks.md 为准。

从 tasks.md 按服务拆分，写入各服务目录 `services/<name>/.ralph/fix_plan.md`：
- fix_plan.md 是 tasks.md 按服务拆分的子集，仅含对应服务任务，格式与主 tasks.md 完全一致
- 生成后自检：任务条目/顺序/依赖与主 tasks.md 完全匹配
...
3. **任务粒度**（硬性校验，替代主观「2-5 分钟」）：
   - 单任务不跨代码层级（数据层/逻辑层/接口层/前端分开）
   - 单任务文件数 ≤3（创建+修改合计）
   - Migration/Proto/配置修改独立成任务，不与业务逻辑合并
   - 单任务测试用例 ≤10 个
   - 前端单任务不混页面开发+路由配置+接口联调三类
   - 粒度分层：Task 含完整 TDD 循环（15-30 分钟）；Task 内 checkbox 步骤 2-5 分钟
4. **TDD 内建**：每个含逻辑代码的 Task 默认包含 RED→GREEN→REFACTOR 步骤，不只是"写代码"；table-driven tests 须覆盖至少 1 正常成功 + 2 异常/边界（参数非法/权限不足/数据不存在/依赖失败）+ spec 定义的所有错误码场景（前端任务 TDD 可选，组件测试/E2E 策略另定）
