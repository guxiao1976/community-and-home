...
export const useDivisionStore = defineStore('division', () => {
  ...
  // Actions
  const fetchDivisions = async (params?: { parent_id?: string; level?: number }) => {
    loading.value = true
    try {
  ...
  const updateDivision = async (id: string, data: {
    name?: string
    sort_order?: number
    status?: number
  }) => {
  ...
  const buildTree = (flatList: AdministrativeDivision[]): AdministrativeDivision[] => {
    const map = new Map<string, AdministrativeDivision>()
