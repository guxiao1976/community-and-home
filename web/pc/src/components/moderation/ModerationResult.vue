<template>
  <div class="moderation-result">
    ...
    <!-- 转换后的前端分层结构显示 -->
    <div class="result-info" v-if="!result.risk_level && (result.traditional || result.smallModel)">
      <el-descriptions :column="2" border>
        <!-- 传统引擎（AC）层 -->
        <el-descriptions-item label="AC引擎" v-if="result.traditional">
          <el-tag :type="result.traditional.passed ? 'success' : 'danger'">
            {{ result.traditional.passed ? '通过' : '拦截' }}
          </el-tag>
          <span v-if="result.traditional.keywords?.length" style="margin-left: 8px; color: #909399;">
            命中: {{ result.traditional.keywords.join(', ') }}
          </span>
          <span v-if="result.traditional.reason" style="margin-left: 8px; color: #909399;">
            {{ result.traditional.reason }}
          </span>
        </el-descriptions-item>
        <!-- 小模型层 -->
        <el-descriptions-item label="小模型" v-if="result.smallModel">
          <el-tag :type="result.smallModel.passed ? 'success' : 'danger'">
            {{ result.smallModel.passed ? '通过' : '拦截' }}
          </el-tag>
          <span style="margin-left: 8px; color: #909399;">
            置信度: {{ (result.smallModel.confidence * 100).toFixed(0) }}%
          </span>
          <span v-if="result.smallModel.reason" style="margin-left: 8px; color: #909399;">
            {{ result.smallModel.reason }}
          </span>
        </el-descriptions-item>
        <!-- 大模型层 -->
        <el-descriptions-item label="大模型" v-if="result.largeModel" :span="2">
          <el-tag :type="result.largeModel.passed ? 'success' : 'danger'">
            {{ result.largeModel.passed ? '通过' : '拦截' }}
          </el-tag>
          <span style="margin-left: 8px; color: #909399;">
            置信度: {{ (result.largeModel.confidence * 100).toFixed(0) }}%
          </span>
          <span v-if="result.largeModel.reason" style="margin-left: 8px; color: #909399;">
            {{ result.largeModel.reason }}
          </span>
        </el-descriptions-item>
      </el-descriptions>
    </div>
...
// 兼容两种响应格式：扁平后端结构 {pass} 和转换后前端结构 {finalResult}
const isPassed = () => {
  if (props.result.finalResult !== undefined) return props.result.finalResult;
  return props.result.pass ?? false;
};
