...
type UserBaseModel interface {
	...
	FindPage(ctx context.Context, keyword string, encryptedPhone string, status *int64, page, pageSize int32) ([]*UserBase, int64, error)
...
func (m *defaultUserBaseModel) Insert(ctx context.Context, data *UserBase) (sql.Result, error) {
	query := fmt.Sprintf(`INSERT INTO %s (id, phone, nickname, avatar_url, real_name, id_card_number, gender, birth_date, status, credit_score, nickname_moderation_status, preferences, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, m.table)
...
func (m *defaultUserBaseModel) FindOne(ctx context.Context, id int64) (*UserBase, error) {
	query := fmt.Sprintf(`SELECT id, phone, nickname, avatar_url, real_name, id_card_number, gender, birth_date, status, credit_score, nickname_moderation_status, preferences, created_at, updated_at,  deleted_at FROM %s WHERE id = ? AND  deleted_at IS NULL`, m.table)
	var resp UserBase
...
func (m *defaultUserBaseModel) FindOneByPhone(ctx context.Context, encryptedPhone string) (*UserBase, error) {
	query := fmt.Sprintf(`SELECT id, phone, nickname, avatar_url, real_name, id_card_number, gender, birth_date, status, credit_score, nickname_moderation_status, preferences, created_at, updated_at,  deleted_at FROM %s WHERE phone = ? AND  deleted_at IS NULL`, m.table)
	var resp UserBase
...
func (m *defaultUserBaseModel) FindByIds(ctx context.Context, ids []int64) ([]*UserBase, error) {
	...
	query := fmt.Sprintf(`SELECT id, phone, nickname, avatar_url, real_name, id_card_number, gender, birth_date, status, credit_score, nickname_moderation_status, preferences, created_at, updated_at,  deleted_at FROM %s WHERE id IN (%s) AND  deleted_at IS NULL`, m.table, strings.Join(placeholders, ","))
	var resp []*UserBase
...
func (m *defaultUserBaseModel) FindPage(ctx context.Context, keyword string, encryptedPhone string, status *int64, page, pageSize int32) ([]*UserBase, int64, error) {
	...
	query := fmt.Sprintf("SELECT id, phone, nickname, avatar_url, real_name, id_card_number, gender, birth_date, status, credit_score, nickname_moderation_status, preferences, created_at, updated_at,  deleted_at FROM %s %s ORDER BY id DESC LIMIT ? OFFSET ?", m.table, where)
...
func (m *defaultUserBaseModel) Update(ctx context.Context, data *UserBase) error {
	query := fmt.Sprintf(`UPDATE %s SET nickname=?, avatar_url=?, gender=?, birth_date=?, status=?, preferences=?, updated_at=? WHERE id=? AND  deleted_at IS NULL`, m.table)
...
func (m *defaultUserBaseModel) UpdateStatus(ctx context.Context, id int64, status int64) error {
	query := fmt.Sprintf(`UPDATE %s SET status=?, updated_at=? WHERE id=? AND  deleted_at IS NULL`, m.table)
...
func (m *defaultUserBaseModel) UpdateNicknameModerationStatus(ctx context.Context, id int64, status int64) error {
	query := fmt.Sprintf(`UPDATE %s SET nickname_moderation_status=?, updated_at=? WHERE id=? AND  deleted_at IS NULL`, m.table)
	_, err := m.conn.ExecCtx(ctx, query, status, time.Now(), id)
	return err
}
...
func (m *defaultUserBaseModel) UpdateRealNameAndIdCard(ctx context.Context, id int64, realName, idCardNumber string) error {
	query := fmt.Sprintf(`UPDATE %s SET real_name=COALESCE(real_name, ?), id_card_number=COALESCE(id_card_number, ?), updated_at=? WHERE id=? AND  deleted_at IS NULL`, m.table)
