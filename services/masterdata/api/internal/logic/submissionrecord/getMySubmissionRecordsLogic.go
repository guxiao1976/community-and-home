...
type GetMySubmissionRecordsLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewGetMySubmissionRecordsLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetMySubmissionRecordsLogic {
	return &GetMySubmissionRecordsLogic{
...
func (l *GetMySubmissionRecordsLogic) GetMySubmissionRecords(req *types.GetSubmissionRecordsReq) (resp *types.GetSubmissionRecordsResp, err error) {
	userId := int64(0)
	...
	records, total, err := l.svcCtx.SubmissionRecordModel.FindBySubmitter(l.ctx, userId, entityType, reviewResult, int64(req.Page), int64(req.PageSize))
	if err != nil {
		return nil, err
	}
