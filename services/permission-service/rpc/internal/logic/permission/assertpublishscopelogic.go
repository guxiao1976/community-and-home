...
// AssertPublishScope 发布数据权限校验（T1.7，统一判据封装）
//
//	scope 解析（复用 resolveUserScope，SHOULD FIX S3）：
...
//	gRPC 超时三层对齐：AssertPublishScope(≤500ms) 内嵌 ResolveScopeAncestors(≤500ms)
//
...
func (l *AssertPublishScopeLogic) AssertPublishScope(in *permissionv1.AssertPublishScopeRequest) (*permissionv1.AssertPublishScopeResponse, error) {
	// 1. 解析用户 community scope → {state, ids}
	state, ids := resolveUserScope(l.ctx, l.svcCtx.UserRoleModel, in.UserId, model.ScopeTypeCommunity)
