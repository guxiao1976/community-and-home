...
type fakeListLostFoundServiceRpc struct {
	communityv1.LostFoundServiceClient
	gotCtx context.Context
	resp   *communityv1.ListLostFoundResponse
}

func (f *fakeListLostFoundServiceRpc) ListLostFound(ctx context.Context, in *communityv1.ListLostFoundRequest, opts ...grpc.CallOption) (*communityv1.ListLostFoundResponse, error) {
	f.gotCtx = ctx
	return f.resp, nil
}

// API List handler 注入 JWT 身份 metadata（T4.6），供 RPC 层 GetDataScopes 读过滤。
// 多视角评审修复：此前只有 Get 有 InjectsIdentity 测试（getlostfoundlogic_test.go），
// ListLostFound 的 CallCtx 注入块无任何测试引用。
//
// SEE: [[tdd-red-evidence-requires-fail-excerpt]] — 行为型断言 RED 摘录留档
// SEE: [[testing-discipline]] — API 层与 Get 同路径注入身份（T4.6 一致）
func TestListLostFound_InjectsIdentity(t *testing.T) {
	fake := &fakeListLostFoundServiceRpc{
		resp: &communityv1.ListLostFoundResponse{Base: responsex.NewBaseResp(), Total: 0},
	}
	svcCtx := &svc.ServiceContext{LostFoundServiceRpc: fake}
	reqCtx := context.WithValue(context.Background(), "user_id", json.Number("42"))

	l := NewListLostFoundLogic(reqCtx, svcCtx)
	resp, err := l.ListLostFound(&types.ListLostFoundReq{CommunityId: 100, Page: 1, PageSize: 10})
