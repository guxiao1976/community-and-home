...
> 自动生成于 2026-06-21 10:18:58 | 数据源: Neo4j 知识图谱 | 每次 `graph-sync.sh` 后刷新
...
| 表名 | 列 |
|------|-----|
| auth_credential | updated_time (datetime), created_time (datetime), credential (varchar), identifier (varchar), identity_type (varchar), user_id (bigint), id (bigint) |
